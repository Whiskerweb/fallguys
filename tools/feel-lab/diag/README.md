...
| `partie.mjs` | Deroulement d'une PARTIE : trois manches tirees au sort et enchainees jusqu'a la victoire. Verifie l'enchainement, la liberation de chaque monde physique, une couronne par partie (et non par manche), des graines toutes distinctes, et le solde final (+8,00 pour deux victoires a 1 USDG). Remplace l'ancien `cycles.mjs`, qui alternait les epreuves a la main. |
| `economie.mjs` | L'un des deux diagnostics qui n'ouvrent pas de navigateur (l'autre est `boutique.mjs`) : il ne juge pas une image mais des nombres. Compare les trois tables de gains rang par rang a des valeurs POSEES A LA MAIN depuis le C#, plus l'invariant `distribue + commission = pot`. Le pendant cote serveur est `backend/npm test`, qui charge reellement `src/economie.js` et compare les deux tables : le lobby annonce ce que le backend paiera, ou le test tombe. |
| `boutique.mjs` | **La BOUTIQUE, sans navigateur.** Un seul article — BabyTrump — et il se gagne en publiant un post sur X. Verifie que le verrou RETIENT sans jamais autoriser (un identifiant hors boutique reste portable : il porte sur ce que J'EQUIPE, pas sur ce qu'un adversaire affiche), que le defaut du catalogue evite le cadenas (`MODELS[0]` EST le personnage verrouille), que l'intention part vers `x.com/intent/post` avec le texte et les hashtags mais AUCUN lien tant que le DNS n'existe pas, et que la reclamation refuse pour trois raisons distinctes — dont une seule se repare en attendant. |
| `boutique-ecran.mjs` | **Le deblocage CLIQUE, dans un vrai navigateur.** Ce que la regle ne porte pas : la vignette cadenassee de la garde-robe mene-t-elle a la boutique, la fenetre qui s'ouvre part-elle bien vers X avec le post ecrit (elle est INTERCEPTEE et refermee — un banc qui charge x.com ne mesure plus le jeu), le bouton attend-il avant d'accepter, et le personnage est-il PORTE a l'arrivee sans un clic de plus. Demande `npx vite build && npx vite preview --port 5274` : `preview` et pas le serveur de dev, dont le rechargement a chaud fausse la mesure. |
| `duel.mjs` | **DEUX navigateurs, un serveur, une partie.** Le seul harnais qui prouve ce que le projet cherche : deux personnes jouant l'une contre l'autre dans un navigateur, sans rien installer. Il demarre lui-meme un serveur de jeu, ouvre deux pages, les fait rejoindre, et verifie qu'elles construisent la MEME carte imposee par le serveur, que chacune affiche l'autre, et que la correction de position converge. Manches longues volontairement : ces pages tournent en rendu logiciel a ~10 % de la vitesse reelle pendant que le serveur, lui, tourne en temps reel. |
| `decor-partage.mjs` | **Le decor est partage.** Une porte cassee par un joueur doit l'etre chez tous. Le montage isole la question : UN SEUL joueur avance, l'autre ne bouge pas — si l'immobile voit les memes portes brisees, le decor vit vraiment pour tout le monde. Surveille aussi qu'aucune coordonnee n'explose : corriger a chaque image un personnage coince contre une porte faisait s'emballer le solveur jusqu'a z = -191 467. |
...
| `hexagone.mjs` | Diagnostic de **L'Hexagone**, en dix-neuf verdicts. Deux questions distinctes, et la seconde ne se deduit pas de la premiere : la tour est-elle JUSTE, et est-elle un JEU ? Cote geometrie, le collider est l'enveloppe convexe des sommets du maillage — donc en principe il ne PEUT pas s'en ecarter, ce qui est exactement la raison de le verifier quand meme. Puis la regle qui fait le mini-jeu : pose au centre d'un hexagone on en recouvre UN, a cheval sur une arete DEUX, sur un sommet TROIS — sans quoi un joueur a cheval resterait debout sur un hexagone qu'il n'a jamais paye. Puis les deux verrous qui tiennent la carte debout : on ne remonte JAMAIS d'un etage (saut plein ET saut suivi d'un plongeon, l'echappatoire evidente), et une chute d'etage a 34 m/s ne declenche aucune culbute parasite. Puis les portees, dans les DEUX sens : un trou d'un hexagone ne doit PAS se franchir au saut, doit se franchir au saut-plongeon meme a fatigue pleine, et un trou de deux ne doit se franchir par aucun moyen. Enfin le verdict qui decide vraiment : TROIS pilotes aux comportements opposes doivent obtenir trois issues etagees — 10 s immobile, 33 s en courant, les 75 s completes en economisant son sol — et la duree a tenir doit tomber DANS cette fenetre. |
...
| `hexacarte.mjs` | La VIGNETTE de **L'Hexagone** pour le carrousel d'entree en manche (`/icons/map-<id>.png`, 640 x 440). Une PRISE DE VUE du jeu, comme les quatre autres : une vignette peinte a cote de captures reelles mentirait sur ce qu'on va jouer. Il creuse les deux etages du haut avant de photographier — une tour intacte ressemble a un gateau, ce sont les trous qui racontent le mini-jeu — et tire au TELEOBJECTIF de loin : a 84 m, les poteaux d'angle du premier plan etaient deux fois plus gros a l'ecran que la tour et remplissaient l'image a eux seuls. Il repousse aussi le brouillard, regle pour une camera de jeu a dix metres et qui delave tout a cent soixante. |
...
| `arrivees.mjs` | **Les LIGNES D'ARRIVÉE.** Sur chaque carte de course : UNE porte (`src/arrivee.js`), posée au centimètre sur `finishZ` — la cote que `main.js` compare —, le damier centré dessus, les DEUX pieds au sol (un rayon Rapier sous chacun, moins de douze centimètres : c'est le verdict « rien ne vole »), trois mètres d'air sous l'enseigne, AUCUNE guirlande libre à moins de douze mètres, et l'enseigne peinte plutôt que le repli au canevas. `vite preview --port 5275`. |
