...
await page.goto('http://127.0.0.1:5273/?lowfx&noassets&skip=scenery&actes=2', { waitUntil: 'domcontentloaded' });
...
await page.evaluate(() => {
  ...
  g.partie = { parcours: [window.__MINIGAMES.find((m) => m.id === 'blockdash')], index: 0, temps: [], chutes: 0 };
  g.startRace();
});
