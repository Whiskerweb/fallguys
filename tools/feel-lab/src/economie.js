...
/**
 * Table des gains pour une mise donnée — `PayoutPolicy.Compute`, repris pas à pas.
 *
 * Règle unique : les survivants de la manche 1 récupèrent leur mise, et ce qui reste
 * après le rake est réparti en bonus entre les finalistes. Le seuil de non-perte tombe
 * donc exactement à la fin de la première manche — c'est l'amortisseur du « mur des 95 %
 * de perdants » décrit dans le spec, et c'est ce que le ticket doit rendre lisible.
 *
...
export function table(mise) {
  ...
  const rake = Math.floor((pot * rakeBp) / 10_000);
