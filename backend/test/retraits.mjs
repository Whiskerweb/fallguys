 ...
 *
 * On n'envoie rien sur Solana ici : signer et diffuser demande une caisse approvisionnee
 ...
 *
 * Le chemin on-chain, lui, se verifie par l'aller-retour devnet decrit dans le README.
 *
...
import { demander } from '../src/solana/retraits.js';
...
titre('Ce qui empeche un retrait de partir');
{
  ...
  await lierWallet(db, petit, 'WalletDuPetit');
  await refuse(
  ...
  await lierWallet(db, pauvre, 'WalletDuPauvre');
  await refuse(
  ...
  await lierWallet(db, presse, 'WalletDuPresse', 0);
  await refuse(
...
titre('Un retrait recevable reserve les fonds');
{
  ...
  await lierWallet(db, eve, 'WalletDEve');

  const r = await demander(db, { userId: eve, montant: 40 * MICROS });
  dit(r.destination === 'WalletDEve',
...
titre('Le deuxieme retrait n\'attend plus');
{
  ...
  await lierWallet(db, franck, 'WalletDeFranck');
  ...
  await lierWallet(db, franck, 'WalletDeFranck', 0);
